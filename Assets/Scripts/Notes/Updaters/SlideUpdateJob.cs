    ...
    private void AutoplayUpdate(ref SlideData slide)
    {
        ...
        switch (NoteHelper.AutoPlayMode)
        {
            ...
            case AutoPlayMode.Random:
                ...
                {
                    // 普通 slide 在先前 RenderStar 的时候计算过 processIdx 可以直接拿来用，wifi 没有这个
        ...
        {
            // Folded slides do not render a star, so starPos is not updated.
            // They must reuse the input generated by the visible copies instead
            // of consuming a DJAuto input slot with the default position.
            if (slide.isFolded) return;
